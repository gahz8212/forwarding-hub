...
### [2026-07-19] 선적 서류(Invoice/Packing List) 대시보드 상태 및 UI 연동 고도화

**1. 대시보드 진행 상태(Stepper) 매핑 논리 수정**
- **문제점:** 기존에는 서류가 업로드만 되어도(`Documents Uploaded`) 바로 스텝퍼가 "내륙 운송" 단계로 넘어가는 UX 혼선이 존재.
- **해결 방안:** `DashboardPage.tsx`의 `getStepIndex` 함수에서 `Documents Uploaded` 상태를 인덱스 `0`('서류 업로드')으로 매핑하도록 수정하여, 두 서류가 **모두 승인 완료**되어 `Documents Verified` 상태가 되기 전까지는 스텝퍼가 넘어가지 않도록 변경.

**2. 서류 삭제 시 "서류 업로드" 상태로 즉시 강등(Revert) 로직 추가**
- **문제점:** 관리자가 서류 보관함에서 승인된 서류 중 하나를 삭제해도, 선적 상태가 "내륙 운송"으로 그대로 남아있는 논리적 오류 발생.
- **해결 방안:** `trackingController.ts`의 `deleteDoc` API에서 서류 삭제 시 DB 경로(NULL 처리)와 승인 여부(0 처리)를 업데이트한 뒤, 현재 상태가 `Documents Uploaded`, `Documents Verified`, 또는 `Trucking`인 경우 즉시 강제로 `Pending Documents`로 상태를 강등시키도록 백엔드 로직 수정.
- 이에 따라 차량 상태(vehicles)도 다시 `Pending`으로 연쇄 복원 및 소켓 알림 이벤트 발송 완료.

**3. 대시보드 서류 업로드 및 확인 패널(UI) 통합 개선**
- **문제점:** 서류를 업로드하는 폼 패널과, 업로드 후 [서류 보기] 버튼을 제공하는 다운로드 패널이 분리/중복되어 나타나거나 아예 사라지는 등 구조적 파편화가 심각.
- **해결 방안:** 두 개의 패널을 제거하고, 직관적인 **단일 통합 패널("선적 서류 업로드 및 확인")**로 병합.
- 서류(인보이스, 패킹리스트)가 **승인 완료(`approved === 1`)**된 경우에는 입력창(`<input type="file" />`)이 `[승인 완료]` 뱃지와 `[서류 보기]` 다운로드 버튼으로 즉시 교체.
- 승인되지 않거나 삭제된 서류의 경우에는 다시 파일 선택창이 나타나며, 어느 하나라도 미승인 상태일 때만 `[서류 업로드 완료하기]` 전송 버튼이 노출되도록 동적 렌더링 적용.

**4. 브라우저 캐싱 문제 우회 (UI 실시간 갱신 버그 픽스)**
